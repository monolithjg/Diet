        ...
        <div key={stepNumber} className="flex-1 flex items-center">
          <div
            className={`
              ...
              mr-2 shrink-0
              ${isCompleted ? 'bg-primary text-white' : isActive ? 'bg-primary-light border-2 border-primary text-primary' : 'bg-gray-200 text-gray-500'}
            `}
          >
            {isCompleted ? (
          ...
          <span 
            className={`text-xs truncate ${isActive ? 'text-primary font-semibold' : isCompleted ? 'text-gray-700' : 'text-gray-500'}`}
          >
            {title}
          </span>
          {stepNumber < totalSteps && (
            <div className={`flex-grow h-0.5 ml-2 ${isCompleted ? 'bg-primary' : 'bg-gray-200'}`}></div>
          )}
        </div>
      );
    });
    ...
    return Array.from({ length: totalSteps }).map((_, index) => (
      <div
        ...
        className={`
          ...
          ${index + 1 === currentStep ? 'bg-primary' : index + 1 < currentStep ? 'bg-primary-light' : 'bg-gray-300'}
        `}
