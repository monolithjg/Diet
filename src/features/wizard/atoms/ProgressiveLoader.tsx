  ...
  if (error) {
    return (
      <div className={cn("min-h-[200px] flex flex-col items-center justify-center p-6 text-center", className)}>
        {/* Error Icon */}
        <div className="w-12 h-12 bg-error-soft rounded-full flex items-center justify-center mb-4">
        ...
        {/* Error Content */}
        <h3 className="text-lg font-semibold text-foreground mb-2">{errorText}</h3>
        ...
        {onRetry && (
          <button
            onClick={onRetry}
            className="px-4 py-2 bg-primary text-primary-foreground rounded-lg font-medium hover:bg-primary-hover transition-colors touch-manipulation min-h-[44px]"
          >
            Try Again
          </button>
        )}
...
function LoadingSkeleton({ variant, text, className }: LoadingSkeletonProps) {
  ...
  return (
    <div className={cn("min-h-[400px] bg-surface rounded-lg shadow-sm", className)}>
      {/* Loading Header */}
